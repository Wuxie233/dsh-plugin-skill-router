...
export default function createHost(api) {
  const { snapshotSessionEvents, createUserMessage, sessionWorkingDirectory, z } = api


const name = 'skill-router'
...
function sessionLookupRoots(ctx, agent) {
  ...
  if (agent !== undefined) {
    try {
      const cwd = sessionWorkingDirectory({
        header: agent.session?.header ?? {},
        events: sessionLog(agent.session),
      })
